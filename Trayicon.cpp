...
LRESULT CTrayIcon::OnTrayNotification(WPARAM wID, LPARAM lEvent)
{
	...
	if (lEvent==WM_RBUTTONUP) {
		...
		if(theApp._enabled)
        {
			pSubMenu->CheckMenuItem(MN_ENABLE, MF_CHECKED|MF_BYCOMMAND);
        }
		else
        {
			pSubMenu->CheckMenuItem(MN_DISABLE, MF_CHECKED|MF_BYCOMMAND);
        }
        if(theApp._working)
        {
            pSubMenu->EnableMenuItem(MN_SETTINGS, MF_DISABLED | MF_GRAYED|MF_BYCOMMAND);
            pSubMenu->EnableMenuItem(MN_STOP, MF_ENABLED|MF_BYCOMMAND);
        }
        else
        {
            pSubMenu->EnableMenuItem(MN_SETTINGS, MF_ENABLED|MF_BYCOMMAND);
            pSubMenu->EnableMenuItem(MN_STOP, MF_DISABLED | MF_GRAYED|MF_BYCOMMAND);
        }
